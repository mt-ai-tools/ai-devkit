...
- Three exceptions: a folder's own README, which documents the folder it sits in; files a tool requires at a fixed location, like `.gitignore` at a repo root, and the template such a file is made from by a copy in place; and pure re-export files (`index.ts`, `api.ts`, and the like) that only re-export what lives in the sibling folders — the moment one gains logic of its own, the exception ends.
